...
contentResponse = document.getElementById("content-response");
...
btnAdd.addEventListener("click", () => {
    ...
    } catch (error) { alert(error) } finally { input.value = "", contentResponse.style.display = "none" }
})
...
function createSelectArray() {
    let itens = "";
    for (let i = 0; i < array.length; i++) { itens += `<option>${array[i]}</option>` }
    return itens;
}
...
function manipularArray() {
    ...
    contentResponse.innerHTML =
        `<p>Ao todo temos ${array.length} valores cadastrados.</p>
